...
void decode_instruction(uint32_t instruction_bits) {
    ...
    switch(instruction.opcode) {
        ...
        case 0x73:
            ...
            break;
        case 0x2a: // Custom instruction lsgt ADDED MANUALLY FOR                                PART3
            write_rtype(instruction);
            break;
    ...
    switch (instruction.rtype.funct3) {
        ...
        case 0x2:
            if (instruction.opcode == 0x2A)
                print_rtype("lsgt", instruction); //LSGT PART3
...
void print_lui(Instruction instruction) {
    // In upper functions code already has checked the OpCode
    printf(LUI_FORMAT, instruction.utype.rd, instruction.utype.imm); //NOT SURE TO EXTEND IT TO 32BITS 
}
...
void print_itype_except_load(char *name, Instruction instruction, int imm) {
    int sign_extended_imm = sign_extend_number(imm, 12); //extending to 32 bit for reading signed numbers
    printf(ITYPE_FORMAT, name, instruction.itype.rd, instruction.itype.rs1, sign_extended_imm);
}

void print_load(char *name, Instruction instruction) {
    int sign_extended_imm = sign_extend_number(instruction.itype.imm, 12); //extending to 32 bit for reading signed numbers
    printf(MEM_FORMAT, name, instruction.itype.rd, sign_extended_imm, instruction.itype.rs1);
}

void print_store(char *name, Instruction instruction) {
    int store_offset = get_store_offset(instruction);
    printf(MEM_FORMAT, name, instruction.stype.rs2, store_offset, instruction.stype.rs1);
}

void print_branch(char *name, Instruction instruction) {
    int branch_offset = get_branch_offset(instruction);
    printf(BRANCH_FORMAT, name, instruction.sbtype.rs1, instruction.sbtype.rs2, branch_offset);
}
